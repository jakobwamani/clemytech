		...
		foreach ($postExist as $post) {
			...
			foreach ($orders as $column) {
				switch ($column['main_slug']) {
					...
					case 'product_title':
						if ($isStripTitle) {
							$postTitle = strip_tags($this->truncateWordwrap($postTitleFull, $stripTitleSize, '...'));
						} else {
							$postTitle = $postTitleFull;
						}
						if ($prodTitleLink) {
							if ($prodTitleQuickView) {
								...
								$data['product_title'] =
									'<a ' .
										( $isStripTitle ? ' class="woobewoo-tooltip" title="' . esc_attr($postTitleFull) . '" ' : '' ) .
										' href="' . esc_url($url) . '"' . ( !$frontend || $prodTitleLinkBlank ? ' target="_blank"' : '' ) .
									'>' .
										esc_html($postTitle) .
									'</a>';
							}
							...
							if ( $isStripTitle ) {
								$data['product_title'] =
									'<span class="woobewoo-tooltip" title="' . esc_attr($postTitleFull) . '">' .
										$postTitle .
									'</span>';
							} else {
								...
								foreach ($variations as $variationIterator => $variation) {
									...
									if ($quantity) {
										...
										$quantityTxt = $stockQuantityText
											...
											: sprintf('<span class="stock-count" data-attribute="%4$s" data-quantity="%1$d">%2$s - %3$s</span> item(s)',
							...
							if ($frontend) {
								...
								if ( $isVariable && ! $hideVariationAttr ) {
									...
									if (count($attributes) > 0 && !$buttonForVariation) {
										$variablesHtml = '<div class="wtbpVarAttributes' . esc_attr($isPopupForVariation ? ' wtbpHidden' : '') . '" data-default-id="' . esc_attr($defaultId) .
								...
								if ( $_wc_price_calculator ) {
									...
									if ($isVariable) {
										...
										add_filter('woocommerce_product_add_to_cart_text', array($this, 'replaceButtonTextVariableProduct'), 30, 1);
									}
									// button for every variation option
									if ($isVariable && !$hideVariationAttr && count($variations) > 0 && $buttonForVariation) {
										$buttons = '<div class="wtbpAddToCartWrapper wtpbVariableButtons" data-product_id="' . $id . '">' .
												$quantityHtml;
									...
									if ($isVariable) {
										remove_filter('add_to_cart_text', array($this, 'replaceButtonTextVariableProduct'), 30, 1);
										remove_filter('woocommerce_product_add_to_cart_text', array($this, 'replaceButtonTextVariableProduct'), 30, 1);
									}
								}
								$varId = '';
							} else {
		...
		if ( $isPro ) {
			foreach ( $this->orderColumns as $column ) {
				...
				if ( array_key_exists( 'add_to_cart_popup', $column ) ) {
					$table .= ViewWtbp::getContent('wootablepressVariablesPopup');
				}
			}
		}
	...
	public function getAttributesHierarchy( $parent = 0, $pre = '' ) {
		...
		if (!empty($attributes)) {
			foreach ($attributes as $attribute) {
				$attributesListArray[$attribute->attribute_id] = array('name' => 'pa_' . $attribute->attribute_name, 'label' => $attribute->attribute_label);
			}
		}
